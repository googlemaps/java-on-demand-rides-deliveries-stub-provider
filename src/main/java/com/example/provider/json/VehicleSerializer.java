 ...
 *
 * <p>
 * From the fleetengine vehicle object, this serializer retrieves the name and
 * the vehicleState.
 */
final class VehicleSerializer implements JsonSerializer<Vehicle> {

  Int32Value kDefaultInt32Value = Int32Value.newBuilder()
      .setValue(0)
      .build();

  @Override
  public JsonElement serialize(Vehicle src, Type typeOfSrc, JsonSerializationContext context) {
    List<Waypoint> waypoints = src.getWaypointsList().stream()
        .map(
            waypoint -> Waypoint.newBuilder()
    ...
    SerializedLatLng serializedLatLng = SerializedLatLng.newBuilder()
        .setLatitude(src.getLastLocation().getLocation().getLatitude())
        .setLongitude(src.getLastLocation().getLocation().getLongitude())
        .build();

    SerializedLastLocation.Builder serializedLastLocationBuilder = SerializedLastLocation.newBuilder()
        .setLatLng(serializedLatLng);

    if (src.getLastLocation().hasHeading()) {
      serializedLastLocationBuilder.setHeading(src.getLastLocation().getHeading().getValue());
    } else {
      serializedLastLocationBuilder.setHeading(0);
    }
    SerializedLastLocation serializedLastLocation = serializedLastLocationBuilder.build();
